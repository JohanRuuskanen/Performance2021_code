    ...
    methods(Access = 'protected')
        ...
        function displayEdges(obj,indices)
            ...
            for i=1:numel(indices)
                ...
                edge.arrow = plot(X,Y,'LineStyle',obj.edgeLineStyle,'LineWidth',1,'HitTest','off','Color',edgeColor);
                if(~obj.undirected)
                ...
                edge.label = text(mean(X)+0.02*normal(1),mean(Y)+0.02*normal(2),edgeLabel,'FontSize',round(obj.fontSize*2.4/3),'HorizontalAlignment','center');%,'Rotation',-90+atan2(normal(2),normal(1))*180/pi);
                hold off;
